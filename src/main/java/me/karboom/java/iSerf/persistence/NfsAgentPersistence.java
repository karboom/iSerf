 ...
 * 目录结构：baseDir/orgId/userId/agent/agentId/
 *   ├── memory/           # 追加写入目录
 *   │   ├── 000001.cbor   # 序号递增的文件
 *   │   ├── 000002.cbor
 *   │   └── ...
 ...
 */
@Slf4j
public class NfsAgentPersistence implements IAgentPersistence {
    ...
    public NfsAgentPersistence(String baseDir) {
    ...
    private String getAgentDir(AgentMetadata metadata) {
        return "%s/%s/%s/agent/%s".formatted(baseDir, metadata.getOrgId(), metadata.getUserId(), metadata.getId());
    }
                        ...
                        try (var userStream = Files.list(orgPath)) {
                            userStream.filter(Files::isDirectory)
                                    .forEach(userPath -> {
                                        ...
                                        // 遍历 agent 目录
                                        var agentTypePath = userPath.resolve("agent");
                                        if (!Files.exists(agentTypePath)) {
                                            return;
                                        }
                                        try (var agentStream = Files.list(agentTypePath)) {
