- 🚀 开始
  - [介绍](zh-cn/开始/介绍.md)
  - [架构](zh-cn/开始/架构.md)

- ✨ 特性
  - [自我进化](zh-cn/特性/自我进化.md)

- 🧩 组件
  - [大模型](zh-cn/组件/大模型/说明.md)
    - [文本](zh-cn/组件/大模型/文本/说明.md)
      - [OpenAI兼容](zh-cn/组件/大模型/文本/OpenAI兼容.md)
      - [Anthropic](zh-cn/组件/大模型/文本/Anthropic.md)
    - [向量](zh-cn/组件/大模型/向量/说明.md)
      - [千问](zh-cn/组件/大模型/向量/千问.md)
  - [服务集群](zh-cn/组件/服务集群/说明.md)
    - [Server](zh-cn/组件/服务集群/Server.md)
    - [WebsocketTransport](zh-cn/组件/服务集群/Websocket集群.md)
    - [Socket.IO客户端](zh-cn/组件/服务集群/SocketIO客户端.md)
  - [团队](zh-cn/组件/团队/说明.md)
  - [智能体](zh-cn/组件/智能体/说明.md)
    - [工具调用](zh-cn/组件/智能体/工具调用.md)
    - [持久化](zh-cn/组件/智能体/持久化.md)
    - [定时任务](zh-cn/组件/智能体/定时任务.md)
    - [记忆管理](zh-cn/组件/智能体/记忆管理.md)

  
- 📦 预设套件
  - [工具](zh-cn/预设套件/工具/说明.md)
    - [本地文件](zh-cn/预设套件/工具/本地文件.md)
    - [数学工具](zh-cn/预设套件/工具/数学工具.md)
    - [SRAG](zh-cn/预设套件/工具/SRAG.md)
  - [团队](zh-cn/预设套件/团队/说明.md)
